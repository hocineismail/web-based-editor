...
import { DONE, SUCCEEDED } from "../constants/constants";
...
const sequenceSlice = createSlice({
  ...
  reducers: {
    ...
    endStep(
      ...
      const { parentStepId } = action.payload;
      // check if the parent has children or not
      const parentStep = state.sequence.Steps.find(
      ...
      if (parentStep && "Transitions" in parentStep) {
        ...
        if (existingTransitionIndex > -1) {
          ...
          parentStep.Transitions[existingTransitionIndex].NextStepId = "Done";
        } else {
          parentStep.Transitions.push({
            result: action.payload.result,
            NextStepId: "Done",
    ...
    updateCurrentStep(
      ...
      state.sequence.Steps.forEach((step) => {
        if (stepId === step.StepId) {
          if ("Function" in step) {
            step.Function = `${action.payload.module}/${action.payload.func}`;
          }
        }
      });
      const { nodes, edges } = parseSteps(state.sequence.Steps, state.nodes);
